...
app.get("/api/stats/daily", requireAuth, (req, res) => {
  ...
  const rows = progressRepo.db
    .prepare(
      "SELECT status, updated_at FROM problem_progress WHERE user_id = ? AND updated_at >= ?",
    )
    .all(req.userId, startMs);
  const map = Object.fromEntries(
    dayKeys.map((k) => [k, { date: k, solved: 0, attempted: 0 }]),
  );
  for (const r of rows) {
...
app.post(
  ...
  requireAdmin,
  (req, res) => {
    const { id, slug, title, badge, order, parentSlug, introText } =
      req.body || {};
    if (!slug || !title)
    ...
    const saved = learningRepo.upsertChapter({
      id: stableId,
      slug,
      title,
      badge,
...
app.delete(
  "/api/admin/learning/chapters/:slug",
  requireAuth,
  requireAdmin,
  (req, res) => {
...
app.post(
  ...
  requireAdmin,
  (req, res) => {
    const { id, chapterId, anchor, title, textMd, videos, order } =
      req.body || {};
    if (!chapterId || !anchor || !title)
      return res
        .status(400)
        .json({ error: "chapterId, anchor, title required" });
    const saved = learningRepo.upsertSection({
      id: id || `ls_${Date.now()}`,
      chapterId,
      anchor,
      title,
...
app.delete(
  "/api/admin/learning/sections/:id",
  requireAuth,
  requireAdmin,
  (req, res) => {
...
// Admin maintenance
app.post(
  "/api/admin/learning/cleanup-orphans",
  requireAuth,
  requireAdmin,
  (req, res) => {
...
app.post(
  ...
  (req, res) => {
    ...
    if (!slug || !parentSlug)
      return res.status(400).json({ error: "slug and parentSlug required" });
    const ch = learningRepo.getChapterBySlug(slug);
    if (!ch) return res.status(404).json({ error: "chapter not found" });
    const parent = learningRepo.getChapterBySlug(parentSlug);
    if (!parent || parent.badge !== "Раздел")
      return res
        .status(400)
        .json({ error: "parent must be an existing Раздел" });
    const saved = learningRepo.upsertChapter({
      id: ch.id,
      slug: ch.slug,
      title: ch.title,
...
app.patch(
  ...
  (req, res) => {
    ...
    const p = problemsRepo.update({
      slug,
      title: title ?? existing.title,
      difficulty: difficulty ?? existing.difficulty,
      frequency: frequency ?? existing.frequency,
      statement: statement ?? existing.statement,
      starterCode: starterCode ?? existing.starter_code,
      tags: tags ?? (existing.tags ? existing.tags.split(",") : []),
      videoUrl: videoUrl ?? existing.video_url,
      solutionMd: solutionMd ?? existing.solution_md,
