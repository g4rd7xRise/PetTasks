...
export default function AdminPage() {
  ...
  const [ghFreq, setGhFreq] = useState<"часто" | "умеренно" | "редко">(
    "умеренно",
  );
  ...
  useEffect(() => {
    if (!sel) return;
    const vEmpty = !(
      (sel as any).video_url && String((sel as any).video_url).trim()
    );
    const sEmpty = !(
      (sel as any).solution_md && String((sel as any).solution_md).trim()
    );
    if (!vEmpty && !sEmpty) return;
    const next = { ...sel } as any;
    if (vEmpty && typeof sel.statement === "string") {
      const m = sel.statement.match(
        /https?:\/\/(?:www\.)?(?:youtube\.com\S*?v=[\w-]{6,}|youtu\.be\/[\w-]{6,})/i,
      );
  ...
  async function updateSelected() {
    if (!sel) return;
    await api(`/api/admin/problems/${sel.slug}`, {
      method: "PATCH",
      body: {
        title: sel.title,
        ...
        starterCode: (sel as any).starter_code || "",
        tags: (sel.tags || "")
          .split(",")
          .map((s: any) => s.trim())
          .filter(Boolean),
        videoUrl: sel.video_url || "",
        solutionMd: sel.solution_md || "",
        published: !!sel.published,
      },
    });
    await loadProblems();
  ...
  async function addTest() {
    ...
    await api(`/api/admin/problems/${sel.slug}/tests`, {
      method: "POST",
      body: { order, input: [], expected: null },
    });
    await loadTests(sel.slug);
  }

  async function updateTest(t: AdminTest) {
    await api(`/api/admin/tests/${t.id}`, {
      method: "PATCH",
      body: {
        order: t.order_index,
        input: JSON.parse(t.input_json),
        expected: JSON.parse(t.expected_json),
      },
    });
  }
  ...
  async function importFromGithub() {
    ...
    async function fetchMarkdown(path: string): Promise<string | null> {
      ...
      try {
        ...
        const hasMdName =
          ...
          meta.name.toLowerCase().endsWith(".md");
        if (
          meta &&
          meta.type === "file" &&
          hasMdName &&
    ...
    function splitIntoTasks(
      ...
      for (const bodyRaw of pieces) {
        ...
        body = body.replace(/\[Вид[её]ообъяснение\]\((https?:[^)]+)\)/gi, "");
        ...
        const codeBlocks =
          body.match(/```(?:ts|tsx|js|jsx)?[\s\S]*?```/gi) ?? [];
        ...
        const lines = cleaned
          ...
          .filter(Boolean);
        const questionLine =
          lines.find((l) => /\?$/.test(l)) || lines[0] || "Задача";
    ...
    async function simulateExpectedOutput(
      code?: string,
    ): Promise<string[] | null> {
      if (!code) return null;
      const logs: string[] = [];
    ...
    for (const fname of paths) {
      try {
        ...
        for (const t of tasks) {
          ...
          const videoUrl = t.videoEmbedId
            ? `https://youtu.be/${t.videoEmbedId}`
            : undefined;
          await api(`/api/admin/problems`, {
            method: "POST",
            body: {
    ...
    <section>
      <Card>
        ...
        <CardContent>
            ...
            <Box sx={{ flex: 1, minWidth: 280 }}>
              ...
              <Stack spacing={1}>
                ...
                <TextField
                  ...
                  onChange={(e) =>
                    setForm({ ...form, solutionMd: e.target.value })
                  }
                />
                <TextField
                  size="small"
                ...
                <FormControlLabel
                  ...
                  label="Опубликовано"
                />
                <Button variant="contained" onClick={saveProblem}>
                  Сохранить
                </Button>
              </Stack>
            </Box>
                ...
                >
                  <div>
                    ...
                    {p.published ? (
                      ...
                      <span style={{ color: "#f0ad4e", fontSize: 12 }}>
                        черновик
                      </span>
                    )}
                  </div>
          ...
          <Divider sx={{ my: 2 }} />
          <Typography variant="subtitle1" sx={{ mb: 1 }}>
            Импорт из GitHub (markdown → задача)
          </Typography>
          <Stack
            direction={{ xs: "column", md: "row" }}
            spacing={1}
          ...
          >
            ...
            <TextField
              ...
              fullWidth
              value={ghFiles}
              onChange={(e) => setGhFiles(e.target.value)}
            />
            <TextField
              size="small"
              select
              label="Сложность"
              value={ghDiff}
              onChange={(e) => setGhDiff(e.target.value as any)}
              sx={{ minWidth: 140 }}
            >
              <MenuItem value="easy">Лёгкая</MenuItem>
              <MenuItem value="medium">Средняя</MenuItem>
            ...
            <TextField
              ...
              label="Частота"
              value={ghFreq}
              onChange={(e) => setGhFreq(e.target.value as any)}
              sx={{ minWidth: 160 }}
            >
              <MenuItem value="часто">Часто</MenuItem>
              <MenuItem value="умеренно">Умеренно</MenuItem>
            ...
            <Button variant="outlined" onClick={importFromGithub}>
              Импортировать
            </Button>
          </Stack>
            ...
            <Box sx={{ mt: 2 }}>
              ...
              <Stack spacing={1} sx={{ mt: 1, mb: 2 }}>
                ...
                <TextField
                  ...
                  onChange={(e) =>
                    setSel({ ...sel!, difficulty: e.target.value as any })
                  }
                >
                  <MenuItem value="easy">Лёгкая</MenuItem>
                  <MenuItem value="medium">Средняя</MenuItem>
                ...
                <TextField
                  ...
                  select
                  label="Частота"
                  value={sel.frequency || "умеренно"}
                  onChange={(e) =>
                    setSel({ ...sel!, frequency: e.target.value as any })
                  }
                >
                  <MenuItem value="часто">Часто</MenuItem>
                  <MenuItem value="умеренно">Умеренно</MenuItem>
                ...
                <TextField
                  ...
                  label="Теги"
                  value={sel.tags || ""}
                  onChange={(e) => setSel({ ...sel!, tags: e.target.value })}
                />
                <TextField
                  size="small"
                  label="Starter code"
                  multiline
                  minRows={3}
                  value={(sel as any).starter_code || ""}
                  onChange={(e) =>
                    setSel({ ...sel!, starter_code: e.target.value as any })
                  }
                />
                <TextField
                  size="small"
                  label="Видео (URL)"
                  value={(sel as any).video_url || ""}
                  onChange={(e) =>
                    setSel({ ...sel!, video_url: e.target.value as any })
                  }
                />
                <TextField
                  size="small"
                  ...
                  multiline
                  minRows={3}
                  value={(sel as any).solution_md || ""}
                  onChange={(e) =>
                    setSel({ ...sel!, solution_md: e.target.value as any })
                  }
                />
                <TextField
                  size="small"
                  ...
                  value={sel.statement}
                  onChange={(e) =>
                    setSel({ ...sel!, statement: e.target.value })
                  }
                />
                <FormControlLabel
                  control={
                    <Checkbox
                      size="small"
                      checked={!!sel.published}
                      onChange={(e) =>
                        setSel({ ...sel!, published: e.target.checked ? 1 : 0 })
                      }
                    />
                  }
                  label="Опубликовано"
                />
                <Button variant="contained" onClick={updateSelected}>
                  Обновить
                </Button>
              </Stack>
              ...
              {tests.map((t) => (
                ...
                >
                  <TextField
                    ...
                    value={t.order_index}
                    onChange={(e) => {
                      const v = { ...t, order_index: Number(e.target.value) };
                      setTests((prev) =>
                        prev.map((x) => (x.id === t.id ? v : x)),
                      );
                      updateTest(v);
                    }}
                    sx={{ width: 80 }}
                  ...
                  <TextField
                    ...
                    value={t.input_json}
                    onChange={(e) => {
                      const v = { ...t, input_json: e.target.value };
                      setTests((prev) =>
                        prev.map((x) => (x.id === t.id ? v : x)),
                      );
                    }}
                  ...
                  <TextField
                    size="small"
                    label="expected JSON"
                    value={t.expected_json}
                    onChange={(e) => {
                      const v = { ...t, expected_json: e.target.value };
                      setTests((prev) =>
                        prev.map((x) => (x.id === t.id ? v : x)),
                      );
                    }}
              ...
              <Button size="small" variant="outlined" onClick={addTest}>
                Добавить тест
              </Button>
            </Box>
          )}
