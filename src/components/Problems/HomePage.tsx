...
export default function HomePage({ githubRawUrl }: Props) {
  ...
  const [daily, setDaily] = useState<
    { date: string; solved: number; attempted: number }[] | null
  >(null);
  useEffect(() => {
    (async () => {
      try {
        const data = await api<
          { date: string; solved: number; attempted: number }[]
  ...
  async function addTodo() {
    ...
    try {
      const created = await api<{
        ...
        text: string;
        completed: boolean;
      }>(`/api/todos`, { method: "POST", body: { text } });
  ...
  async function toggleTodo(id: string, completed: boolean) {
    ...
    try {
      await api(`/api/todos/${id}`, { method: "PATCH", body: { completed } });
    } catch {}
  }
  ...
  async function clearCompleted() {
    ...
    try {
      await Promise.all(
        doneIds.map((id) => api(`/api/todos/${id}`, { method: "DELETE" })),
      );
    } catch {}
  }
  ...
  useEffect(() => {
    (async () => {
      try {
        const items =
          await api<{ id: string; text: string; completed: boolean }[]>(
            `/api/todos`,
          );
        setTodos(items);
        setTodoError(null);
    ...
    <section>
      ...
      >
        <Box>
          ...
          >
            <CardHeader
              title="Тренировочный хаб"
              subheader="Работайте над задачами и отслеживайте прогресс"
            />
            <CardContent>
              <Stack spacing={1.5}>
                <Typography variant="body2">
                  Ctrl+Enter — запустить тесты
                </Typography>
                <Typography variant="body2">
                  Задача отмечается решенной при прохождении всех тестов
                </Typography>
                <Typography variant="body2">
                  Можно импортировать задачи с GitHub (см. источник в настройке)
                </Typography>
              </Stack>
            </CardContent>
          </Card>
                  ...
                  series={[
                    {
                      data: [
                        ...
                        { id: 1, value: stats.attempted, label: "Попытки" },
                        {
                          id: 2,
                          value: Math.max(
                            stats.total - stats.solved - stats.attempted,
                            0,
                          ),
                          label: "Осталось",
                        },
            ...
            {daily && daily.length > 0 ? (
              <LineChart
                ...
                series={[
                  {
                    id: "attempted",
                    data: daily.map((d) => d.attempted),
                    label: "Попытки",
                  },
                  {
                    id: "solved",
                    data: daily.map((d) => d.solved),
                    label: "Решено",
                  },
                ]}
                height={260}
      ...
      >
              ...
              >
                <Stack direction="row" spacing={1} alignItems="center">
                  ...
                  <Typography variant="caption" sx={{ opacity: 0.6, ml: 1 }}>
                    {hasToken ? "Авторизован" : "Нет токена"}
                  </Typography>
                </Stack>
                <ToggleButtonGroup
                  value={filter}
                      ...
                      >
                        ...
                        {editingId === t.id ? (
                          <TextField
                            ...
                            onBlur={() => {
                              updateTodoText(t.id, editingText);
                              setEditingId(null);
                            }}
                            onKeyDown={(e) => {
                              if (e.key === "Enter") {
        ...
        <Box>
          ...
          >
            ...
            <CardContent>
              <Typography variant="body2">
                — Старайтесь решать задачи маленькими шагами
              </Typography>
              <Typography variant="body2">
                — Запускайте тесты как можно чаще
              </Typography>
              <Typography variant="body2">
                — Отмечайте прогресс для мотивации
              </Typography>
            </CardContent>
          </Card>
        </Box>
