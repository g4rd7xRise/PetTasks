...
export default function ProblemDetail({ slug, githubRawUrl, onBack }: Props) {
  ...
  const [leftTab, setLeftTab] = useState<
    "desc" | "materials" | "solution" | "video" | "answer"
  >("desc");
  ...
  function checkAnswer() {
    const user = answerText
      ...
      .map((s) => s.trim())
      .filter(Boolean);
    if (!user.length || !expectedLines.length) {
      setSnack("Нет данных для проверки");
      return;
    }
    const ok =
      user.length === expectedLines.length &&
      user.every(
        (l, i) => normalizeLine(l) === normalizeLine(expectedLines[i]),
      );
    setAttempts((a) => a + 1);
    if (ok) {
      setSolved(true);
  ...
  function runWithFilter(filterId?: string) {
    ...
    try {
      ...
      if (typeof fn !== "function")
        throw new Error(`Функция "${problem.functionName}" не найдена`);
      ...
      if (filterId)
        testsToRun = testsToRun.filter((tc: TestCase) => tc.id === filterId);
      ...
      for (const tc of testsToRun) {
        if (Date.now() - startTs > timeoutMs) {
          ...
          results.push({
            ...
            actual: null,
            error: `Превышен лимит ${timeoutMs}ms`,
          });
          continue;
        }
        try {
  ...
  async function addToTodo() {
    ...
    try {
      await api(`/api/todos`, {
        method: "POST",
        body: { text: `Решить: ${problem.title}` },
      });
  ...
  return (
    <section>
      {problem && (
        ...
        >
          ...
          >
            ...
            <Chip
              size="small"
              label={problem.frequency || "умеренно"}
              variant="outlined"
            />
            ...
            <Typography variant="caption" sx={{ opacity: 0.8 }}>
              Последнее:{" "}
              {new Date(Math.max(0, lastUpdatedAt)).toLocaleDateString()}
            </Typography>
          </Stack>
          <Stack direction="row" spacing={1}>
            <Button size="small" variant="outlined" onClick={addToTodo}>
              Добавить в To‑Do
            </Button>
          </Stack>
        </Box>
      ...
      <div style={{ marginBottom: 8, display: "flex", gap: 8 }}>
        ...
        <div style={{ alignSelf: "center", fontSize: 12, color: "#555" }}>
          Попыток: {attempts} • Статус: {solved ? "Решена" : "Не решена"}
        </div>
      </div>

      <div
        className="split-grid full-height"
        style={{
          gridTemplateColumns: `${leftWidth}% 8px ${midWidth}% 8px ${rightWidth}%`,
        }}
      >
        {/* Panel 1 with tabs */}
        <div
            ...
            <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
              <button
                ...
                style={{
                  ...
                  marginBottom: 4,
                  backgroundColor:
                    leftTab === "desc" ? "var(--resizer)" : "transparent",
                }}
              ...
              <button
                ...
                style={{
                  ...
                  marginBottom: 4,
                  backgroundColor:
                    leftTab === "materials" ? "var(--resizer)" : "transparent",
                }}
              ...
              <button
                ...
                style={{
                  ...
                  marginBottom: 4,
                  backgroundColor:
                    leftTab === "solution" ? "var(--resizer)" : "transparent",
                }}
              ...
              <button
                ...
                style={{
                  ...
                  marginBottom: 4,
                  backgroundColor:
                    leftTab === "video" ? "var(--resizer)" : "transparent",
                }}
            ...
            {leftTab === "answer" && problem.tags.includes("eventloop") && (
              <div>
                <div style={{ fontSize: 12, opacity: 0.8, marginBottom: 6 }}>
                  Введите ожидаемый порядок строк (по одной на строку):
                </div>
                <textarea
                  value={answerText}
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                  ...
                  <Button
                    variant="outlined"
                    size="small"
                    onClick={() => setAnswerText(expectedLines.join("\n"))}
                  >
                    Заполнить образцом
                  </Button>
                </div>
        ...
        <div
          className="v-resizer"
          onMouseDown={(e) => startDragResizer(1, e)}
        />
          ...
          <div className="editor-header">
            ...
            <div className="editor-header-right">
              ...
              <Button
                variant="contained"
                size="small"
                disabled={running || problem.tags.includes("eventloop")}
                ...
                <MenuItem disableRipple>
                  <FormControlLabel
                    control={
                      <Checkbox
                        size="small"
                        checked={shuffleTests}
                        onChange={(e) => setShuffleTests(e.target.checked)}
                      />
                    }
        ...
        <div
          className="v-resizer"
          onMouseDown={(e) => startDragResizer(2, e)}
        />
        ...
        >
          ...
          <div className="panel-content">
            ...
            {problem.tags.includes("eventloop") && rightTab === "answer" && (
              <Paper
                variant="outlined"
                sx={{ p: 1.5, borderRadius: 2, bgcolor: "transparent" }}
              >
