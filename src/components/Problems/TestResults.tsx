    ...
    <div style={{ padding: 12 }}>
      ...
      {results.map((result, index) => (
        ...
        >
          ...
          >
            {result.passed ? "✅" : "❌"} Тест {result.testId} (#{index + 1})
            //Костыль
          </div>

          <div style={{ fontSize: 12 }}>
            <div>
              <strong>Вход:</strong> {JSON.stringify(result.input)}
            </div>
            <div>
              <strong>Ожидается:</strong> {JSON.stringify(result.expected)}
            </div>
            <div>
