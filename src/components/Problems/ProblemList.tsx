...
export default function ProblemList({ githubRawUrl, onOpenProblem }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    const list = problems.filter((p) => {
      ...
      const matchesFreq =
        frequency === "all" || (p.frequency || "умеренно") === frequency;
      return matchesQuery && matchesDiff && matchesFreq;
    });
    if (sort === "frequency") {
      return list.sort(
        (a, b) =>
          freqRank[b.frequency || "умеренно"] -
          freqRank[a.frequency || "умеренно"],
      );
    } else if (sort === "difficulty-asc") {
      return list.sort(
        (a, b) => diffRank[a.difficulty] - diffRank[b.difficulty],
      );
    } else if (sort === "difficulty-desc") {
      return list.sort(
        (a, b) => diffRank[b.difficulty] - diffRank[a.difficulty],
      );
    }
    return list;
    ...
    <section>
      ...
      >
        ...
        <Select
          size="small"
          value={frequency}
          onChange={(e) => setFrequency(String(e.target.value))}
          sx={{ minWidth: 160 }}
        >
