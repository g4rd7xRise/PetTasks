  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const parentFromUrl = url.hash.includes("?")
          ? new URLSearchParams(url.hash.split("?")[1]).get("parent")
          : null;
  ...
  async function persistAll() {
    try {
      ...
      for (let idx = 0; idx < sections.length; idx++) {
        ...
        await api("/api/admin/learning/sections", {
          method: "POST",
          body: {
            id: s.id || `ls_${Date.now()}_${idx}`,
            chapterId: chId,
            anchor: s.anchor,
      ...
      alert(
        "Не удалось сохранить изменения. Проверьте, что у вас права администратора и сервер запущен.",
      );
    }
  }
    ...
    <Box sx={{ py: 4 }}>
      {isAdmin && (
        <Alert severity="info" sx={{ mb: 2 }}>
          Подсказки Markdown: заголовки #, **жирный**, _курсив_, списки -,
          нумерация 1., код `inline` и блоки ```. Поддерживается GitHub Flavored
          Markdown (таблицы, чекбоксы).
        </Alert>
      )}
      <Stack
      ...
      >
        <Card
          variant="outlined"
          sx={{ width: { xs: "100%", md: 280 }, position: "sticky", top: 88 }}
        >
          <List
            subheader={
              <ListSubheader component="div">Содержание</ListSubheader>
            }
          >
            {sections.map((s, i) => (
              <ListItemButton
              ...
              <Box key={s.anchor} id={s.anchor} sx={{ mb: 4 }}>
                ...
                >
                  ...
                  {isAdmin && editing && (
                    <Tooltip title="Удалить блок">
                      <IconButton size="small" onClick={() => removeSection(i)}>
                ...
                {editing ? (
                  <Stack spacing={1} sx={{ mb: 1 }}>
                    <MarkdownToolbar
                      get={() => s.text || ""}
                      set={(next) => updateSection(i, { text: next })}
                    />
                    <TextField
                      label="Видео 1 (YouTube или embed)"
                      value={s.embedInput || s.embed || ""}
                      onChange={(e) =>
                        updateSection(i, {
                          embedInput: e.target.value,
                          embed: toEmbed(e.target.value),
                        })
                      }
                      fullWidth
                      size="small"
                    />
                    <TextField
                      label="Видео 2 (необязательно)"
                      value={s.embed2Input || s.embed2 || ""}
                      onChange={(e) =>
                        updateSection(i, {
                          embed2Input: e.target.value,
                          embed2: toEmbed(e.target.value),
                        })
                      }
                      fullWidth
                      size="small"
                ...
                {editing ? (
                  <TextField
                    multiline
                    minRows={3}
                    placeholder="Текст раздела (поддерживается Markdown)"
                    value={s.text || ""}
      ...
      {isAdmin && (
        <Tooltip
          title={
            editing ? "Сохранить изменения" : "Редактировать контент главы"
          }
        >
          <Fab
            color="primary"
      ...
      sections: [
        {
          anchor: "intro",
          title: "Введение",
          text: "Зачем HTML и как мыслить разметкой.",
        },
        {
          anchor: "course1",
          title: "Курс: HTML & CSS подробно",
          embed: "https://www.youtube.com/embed/dQw4w9WgXcQ",
          text: "Полный курс с практикой.",
        },
        {
          anchor: "seo",
          title: "SEO и доступность",
          text: "Основы SEO, A11y и проверка страниц.",
        },
        {
          anchor: "practice",
          title: "Практика",
          text: "Сделайте семантическую разметку небольшого лендинга.",
        },
      ],
    },
    ch2: {
      title: "Глава 2. CSS",
      subtitle: "Селекторы, каскад, адаптивность",
      sections: [
        { anchor: "intro", title: "Введение", text: "Обзор CSS и каскада." },
      ],
    },
  };
  return (
      ...
      sections: [
        {
          anchor: "intro",
          title: "Введение",
          text: "Эта глава ещё пустая. Добавьте блоки через режим редактирования.",
        },
      ] as SectionData[],
    }
  );
