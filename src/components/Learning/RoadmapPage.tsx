...
export default function RoadmapPage() {
      ...
      if (Array.isArray(rows)) {
        ...
        setChapters(
          unique.map((r) => ({
            ...
            title: r.title,
            badge: r.badge,
            parentSlug: (r as any).parent_slug || null,
            introText: (r as any).intro_text || "",
            order: (r as any).order_num ?? 0,
            body: <Placeholder />,
          })),
        );
      } else {
  ...
  async function saveChapterToApi(id: string) {
    ...
    setBusy(true);
    try {
      await api("/api/admin/learning/chapters", {
        method: "POST",
        body: {
          id: (ch as any).dbId,
          slug: ch.id,
          title: editingTitle || ch.title,
          badge: ch.badge || "Глава",
          parentSlug: (ch as any).parentSlug || null,
          introText: (ch as any).introText || "",
          order: 0,
        },
      });
      ...
      for (let idx = 0; idx < list.length; idx++) {
        ...
        await api("/api/admin/learning/chapters", {
          ...
          body: {
            ...
            title: it.title,
            badge: it.badge,
            parentSlug: (it as any).parentSlug || null,
            introText: (it as any).introText || "",
            order: idx,
          },
        });
      }
  ...
  function optimisticReorderSections(newSectionIds: string[]) {
    ...
    for (const ch of chapters) {
      ...
      if (parent == null && (ch as any).badge === "Раздел")
        sectionById[ch.id] = ch;
      else if (parent) {
    ...
    <Box sx={{ py: 4 }}>
      <Stack spacing={2} sx={{ mb: 2 }}>
        ...
        <Typography sx={{ opacity: 0.8 }}>
          Материал структурирован по разделам и главам. Раздел содержит краткое
          описание, затем идут главы.
        </Typography>
      </Stack>
        ...
        <Stack direction="row" spacing={1} sx={{ mb: 1 }}>
          <Button
            ...
            onClick={async () => {
              setBusy(true);
              try {
                await api("/api/admin/learning/cleanup-orphans", {
                  method: "POST",
                });
                const rows = await api<any[]>("/api/learning/chapters");
                setChapters(
      ...
      <Card variant="outlined" sx={{ borderRadius: 2 }}>
        <CardContent>
          <Stack spacing={1.5}>
            {chapters.length === 0 && (
              <Typography sx={{ opacity: 0.8 }}>
                Нет данных. Добавьте раздел с помощью кнопки ниже.
              </Typography>
            )}
            {(() => {
              ...
              const sectionIdsSorted = chapters
                .filter(
                  (c) =>
                    (c as any).parentSlug == null &&
                    (c as any).badge === "Раздел",
                )
                .sort((a: any, b: any) => (a.order ?? 0) - (b.order ?? 0))
                .map((c) => c.id);
              const allKeys = Object.keys(groups);
              const otherKeys = allKeys.filter(
                (k) => !sectionIdsSorted.includes(k),
              );
              const orderedGroupKeys = [...sectionIdsSorted, ...otherKeys];
              return orderedGroupKeys.map((groupKey) => {
                const items = groups[groupKey];
                const parent = items.find(
                  (i) => (i as any).parentSlug == null && i.id === groupKey,
                );
                const isSection =
                  !!parent && (parent as any).badge === "Раздел";
                const title = isSection
                  ? parent.title
                  : items[0]?.title || groupKey;
                return (
                  <Accordion
                    key={groupKey}
                    expanded={expanded === groupKey}
                    onChange={(_, is) => setExpanded(is ? groupKey : false)}
                    disableGutters
                    ...
                    onDrop={() => {
                      ...
                      const sectionIds = newOrder.filter(
                        (k) =>
                          !!groups[k].find(
                            (i) => (i as any).parentSlug == null && i.id === k,
                          ),
                      );
                      const newCh = [...chapters].sort((a, b) => {
                        const ia = sectionIds.indexOf(
                          (a as any).parentSlug ? (a as any).parentSlug : a.id,
                        );
                        const ib = sectionIds.indexOf(
                          (b as any).parentSlug ? (b as any).parentSlug : b.id,
                        );
                        return ia - ib;
                      });
                      setChapters(newCh);
                      // persist only sections order
                      const topSections = sectionIds
                        ...
                        >
                          {isSection && editingId === parent?.id ? (
                            <TextField
                              ...
                              onBlur={async () => {
                                await saveChapterToApi(parent!.id);
                              }}
                              onKeyDown={async (e) => {
                                if (e.key === "Enter") {
                                  e.preventDefault();
                          ...
                          <Stack direction="row" spacing={1}>
                            ...
                            <IconButton
                              ...
                              onClick={(e) => {
                                ...
                                [newOrderIds[idx - 1], newOrderIds[idx]] = [
                                  newOrderIds[idx],
                                  newOrderIds[idx - 1],
                                ];
                                optimisticReorderSections(newOrderIds);
                                const topSections = newOrderIds
                                  .map((id) =>
                                ...
                                (async () => {
                                  try {
                                    await persistOrder(topSections);
                                  } finally {
                                    setBusy(false);
                                  }
                                })();
                            ...
                            <IconButton
                              ...
                              onClick={(e) => {
                                ...
                                const sectionIds = chapters
                                  ...
                                  .map((c) => c.id);
                                const idx = sectionIds.indexOf(groupKey);
                                if (idx < 0 || idx >= sectionIds.length - 1)
                                  return;
                                const newOrderIds = [...sectionIds];
                                [newOrderIds[idx + 1], newOrderIds[idx]] = [
                                  newOrderIds[idx],
                          ...
                          <>
                            ...
                            {editingId === parent?.id ? (
                              <>
                                ...
                                />
                                <TextField
                                  placeholder="Краткое описание раздела (Markdown)"
                                  value={(parent as any).introText || ""}
                                  onChange={(e) =>
                                    setChapters((list) =>
                                      list.map((c) =>
                                        c.id === parent!.id
                                          ? { ...c, introText: e.target.value }
                                          : c,
                                      ),
                                    )
                                  }
                            ...
                            {items
                              .filter((i) => (i as any).parentSlug != null)
                              .map((child) => (
                                <Stack
                                  key={child.id}
                                  direction="row"
                                  ...
                                  onDrop={() => {
                                    ...
                                    const children = items.filter(
                                      (i) => (i as any).parentSlug != null,
                                    );
                                    const srcIdx = children.findIndex(
                                      (c) => c.id === dragId,
                                    );
                                    const dstIdx = children.findIndex(
                                      (c) => c.id === child.id,
                                    );
                                    if (srcIdx < 0 || dstIdx < 0) return;
                                    const newChildren = [...children];
                                    const [moved] = newChildren.splice(
                                      srcIdx,
                                      1,
                                    );
                                    newChildren.splice(dstIdx, 0, moved);
                                    // apply back to chapters state
                                    const others = chapters.filter(
                                      (c) => (c as any).parentSlug !== groupKey,
                                    );
                                    setChapters([
                                      ...others,
                                      ...newChildren,
                                  ...
                                  >
                                    {isAdmin && editingChildId === child.id ? (
                                      <TextField
                                        ...
                                        fullWidth
                                        onChange={(e) =>
                                          setEditingChildTitle(e.target.value)
                                        }
                                        onBlur={async () => {
                                          setChapters((list) =>
                                            list.map((c) =>
                                          ...
                                          try {
                                            await api(
                                              "/api/admin/learning/chapters",
                                              {
                                                method: "POST",
                                                body: {
                                                  id: (child as any).dbId,
                                                  slug: child.id,
                                                  title: editingChildTitle,
                                                  badge: child.badge || "Глава",
                                                  parentSlug: (child as any)
                                                    .parentSlug,
                                                  order: 0,
                                                },
                                              },
                                        ...
                                        <>
                                          ...
                                          <Button
                                            ...
                                            onClick={() => {
                                              ...
                                              setEditingChildTitle(child.title);
                                            }}
                                          >
                                            Переименовать
                                          </Button>
                                          ...
                                          <Button
                                            ...
                                            color="primary"
                                            onClick={() => {
                                              window.location.hash = `learn-${child.id}?parent=${encodeURIComponent(String(groupKey))}`;
                                            }}
                                            endIcon={
                                              <ArrowForwardIosIcon
                                                sx={{ fontSize: 14 }}
                            ...
                            >
                                      ...
                                      try {
                                        await api(
                                          "/api/admin/learning/chapters",
                                          {
                                            method: "POST",
                                            body: {
                                              id: (single as any).dbId,
                                              slug: single.id,
                                              title: editingChildTitle,
                                              badge: single.badge || "Глава",
                                              parentSlug:
                                                (single as any).parentSlug ||
                                                null,
                                              order: 0,
                                            },
                                          },
                              ...
                              >
                                  ...
                                  (editingChildId === single.id ? (
                                    ...
                                    <>
                                      <Button
                                        size="small"
                                        onClick={() => {
                                          setEditingChildId(single.id);
                                          setEditingChildTitle(single.title);
                                        }}
                                      >
                                        Переименовать
                                      </Button>
                                      ...
                                      <Button
                                        size="small"
                                        onClick={async () => {
                                          const parentSlug = prompt(
                                            "Введите slug раздела (например, ch0-intro-section):",
                                          );
                                          if (!parentSlug) return;
                                          setBusy(true);
                                          try {
                                            await api(
                                              "/api/admin/learning/assign-parent",
                                              {
                                                method: "POST",
                                                body: {
                                                  slug: single.id,
                                ...
                                <Button
                                  ...
                                  color="primary"
                                  onClick={() => {
                                    window.location.hash = `learn-${single.id}`;
                                  }}
                                  endIcon={
                                    <ArrowForwardIosIcon
                                      sx={{ fontSize: 14 }}
